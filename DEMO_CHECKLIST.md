...
Use this checklist right before a live demo. Keep the demo browser-first and avoid changing config during rehearsal.

## Environment

- `.env` is present locally and stable for the session.
- Python virtual environment is activated.
- Recommended runtime is Python 3.10-3.12. Python 3.14 can still emit a torch/sentence-transformers warning.
...
- Do not run approve-index unless you intentionally want additive catalog writes during the demo.
...
- Fallback scope is shopper-first demo only; seller/admin write actions are optional.
