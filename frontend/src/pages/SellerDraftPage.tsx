...
export function SellerDraftPage() {
    ...
    const draftsQuery = useQuery({ queryKey: ["seller-drafts"], queryFn: () => listSellerDrafts({ limit: 12 }) });

    const createMutation = useMutation({
        mutationFn: (payload: SellerDraftPayload) => createSellerDraft(payload),
        onSuccess: (response) => {
    ...
    const validateMutation = useMutation({
        mutationFn: (draftId: string) => validateSellerDraft(draftId),
        onSuccess: (response) => {
            setActiveDraft(response.draft);
            queryClient.invalidateQueries({ queryKey: ["seller-drafts"] });
        },
    ...
    const previewMutation = useMutation({
        mutationFn: (draftId: string) => previewSellerDraftIndexing(draftId),
        onSuccess: (response) => {
    ...
    if (draftsQuery.isLoading) {
        return <LoadingState title="Loading seller tools" message="Checking whether seller draft staging is enabled." />;
    }

    if (draftsQuery.error instanceof Error) {
        return <ErrorState title="Seller tools unavailable" message={draftsQuery.error.message} />;
    }

    if (!draftsQuery.data?.enabled) {
        return (
            <section className="space-y-5">
                <header className="panel p-6">
                    <StatusBadge tone="amber">Disabled by default</StatusBadge>
                    <h1 className="mt-3 text-2xl font-medium text-[var(--ink-strong)]">Seller tools are off</h1>
                    <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--ink-soft)]">
                        Batch 14.5 keeps seller catalog writes behind `ENABLE_SELLER_TOOLS=false` by default. Existing shopper
                        demo and recommendation flows remain unchanged.
                    </p>
                </header>
                <EmptyState
        ...
        <section className="space-y-5">
            ...
            <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr),420px]">
                <div className="space-y-5">
                    <SellerDraftForm isCreating={createMutation.isPending} onCreate={(payload) => createMutation.mutate(payload)} />
                ...
                <aside className="space-y-4">
                    ...
                    <section className="panel p-5">
                        ...
                        <button
                            ...
                            disabled={!canApprove || approveMutation.isPending || !displayedDraft}
                            type="button"
